...
use tower_http::{
    compression::CompressionLayer, cors::CorsLayer, services::ServeFile,
...
#[derive(Clone)]
struct E2EClientMeta {
    session_id: Option<String>,
    client_id: String,
    client_label: String,
    client_kind: E2EClientKind,
}
...
fn parse_client_kind(value: Option<&str>, default_kind: E2EClientKind) -> E2EClientKind {
    match value.map(|raw| raw.trim().to_ascii_lowercase()) {
        Some(kind) if kind == "remote" => E2EClientKind::Remote,
        Some(kind) if kind == "control_plane" || kind == "control-plane" => {
            E2EClientKind::ControlPlane
        }
        Some(kind) if kind == "visualizer" => E2EClientKind::Visualizer,
        Some(kind) if kind == "server" => E2EClientKind::Server,
        _ => default_kind,
    }
}

fn command_client_kind(command: &RemoteCommand) -> E2EClientKind {
    if let Some(client_kind) = &command.client_kind {
        return client_kind.clone();
    }

    match command
        .device_type
        .as_ref()
        .unwrap_or(&DeviceType::MobileRemote)
    {
        DeviceType::ControlPlane => E2EClientKind::ControlPlane,
        DeviceType::MobileRemote => E2EClientKind::Remote,
        DeviceType::System => E2EClientKind::Server,
    }
}

fn resolve_e2e_client_meta(
    session_id: Option<String>,
    client_id: Option<String>,
    client_label: Option<String>,
    client_kind: Option<String>,
    default_kind: E2EClientKind,
) -> E2EClientMeta {
    let resolved_kind = parse_client_kind(client_kind.as_deref(), default_kind.clone());
    let fallback_label = match resolved_kind {
        E2EClientKind::Remote => "remote",
        E2EClientKind::ControlPlane => "control-plane",
        E2EClientKind::Visualizer => "visualizer",
        E2EClientKind::Server => "server",
    };

    E2EClientMeta {
        session_id,
        client_id: client_id.unwrap_or_else(|| fallback_label.to_string()),
        client_label: client_label.unwrap_or_else(|| fallback_label.to_string()),
        client_kind: resolved_kind,
    }
}
...
fn snapshot_payload(
    ...
    let snapshot = serde_json::to_value(app_state_sync.get_e2e_state_snapshot(connection_phase))
...
impl Drop for SseDisconnectGuard {
    fn drop(&mut self) {
        self.app_state_sync.record_server_e2e_event(
            "server_sse_client_disconnected",
            serde_json::json!({ "transport": self.transport }),
            self.session_id.as_deref(),
            Some(self.client_id.as_str()),
            Some(self.client_label.as_str()),
            Some(self.client_kind.clone()),
        );
    }
}
...
pub async fn start_server(app_handle: AppHandle, app_state_sync: Arc<AppStateSync>, port: u16) {
    ...
    let compressed_api = Router::new()
        ...
        .route("/api/e2e/config", get(get_e2e_config))
        .route("/api/e2e/report", post(handle_e2e_report))
        .route("/api/e2e/last-report", get(get_last_e2e_report))
        .route("/api/e2e/session/start", post(start_e2e_session))
        .route(
            "/api/e2e/session/:session_id/events",
            get(get_e2e_session_events),
        )
        .route(
            "/api/e2e/session/:session_id/summary",
            get(get_e2e_session_summary),
        )
    ...
    let Some((listener, addr)) = bound_listener else {
        eprintln!(
            "LAN server could not bind any port in range {}..{}",
            port,
            port.saturating_add(20)
        );
        return;
    };
...
async fn list_images(
    ...
    // Log request details
    eprintln!(
        "[Server] [list_images] Request received - folder: '{}', params: {:?}",
        folder_path, params
    );
    ...
    if folder_path.is_empty() {
        ...
        return Err(ApiErrorResponse::bad_request(
            "Folder parameter is required and cannot be empty",
        ));
    }
    ...
    let resolved = if let Some(subpath) = folder_path.strip_prefix("$RESOURCES/") {
        ...
        eprintln!(
            "[Server] [list_images] Resolving resource path: '{}'",
            subpath
        );
            ...
            .resolve(subpath, BaseDirectory::Resource)
        {
            Ok(p) => {
                ...
                eprintln!(
                    "[Server] [list_images] Resource '{}' resolved to: '{}'",
            ...
            Err(e) => {
                ...
                return Err(ApiErrorResponse::resource_resolution_error(
                    &folder_path,
                    &e.to_string(),
                ));
            }
        ...
        eprintln!(
            "[Server] [list_images] Resolving file system path: '{}'",
            folder_path
        );
    ...
    match std::fs::read_dir(path) {
        Ok(entries) => {
            ...
            for entry_result in entries {
                match entry_result {
                    ...
                    Err(e) => {
                        eprintln!(
                            "[Server] [list_images] WARNING: Error reading directory entry: {}",
                            e
                        );
                        error_count += 1;
                    }
        ...
        Err(e) => {
            ...
            match e.kind() {
                std::io::ErrorKind::PermissionDenied => Err(ApiErrorResponse::forbidden(&format!(
...
async fn serve_image(Query(params): Query<HashMap<String, String>>, request: Request) -> Response {
...
async fn serve_youtube_player(State(state): State<AppState>, request: Request) -> Response {
    serve_file_response(
        state.dist_path.join("youtube_player.html"),
        request,
        Vec::new(),
    )
    .await
...
async fn handle_command(
    ...
    let device_type = payload
        ...
        .clone()
        .unwrap_or(DeviceType::MobileRemote);
    let client_kind = command_client_kind(&payload);
    state.app_state_sync.record_server_e2e_event(
        "server_command_received",
        serde_json::json!({
            "command": payload.command.clone(),
            "deviceType": payload.device_type.clone(),
            "payload": payload.payload.clone(),
        }),
        payload.session_id.as_deref(),
        payload.client_id.as_deref(),
        payload.client_label.as_deref(),
        Some(client_kind.clone()),
    );
        ...
        "set-messages" => {
            if let Some(p) = &payload.payload {
                ...
                if let Ok(messages) = serde_json::from_value::<Vec<MessageConfig>>(p.clone()) {
                    ...
                    let messages: Vec<MessageConfig> = arr
                        .iter()
                        .enumerate()
                        .filter_map(|(i, v)| {
    ...
    state.app_state_sync.record_server_e2e_event(
        "server_command_applied",
        snapshot_payload(
            state.app_state_sync.as_ref(),
            None,
            serde_json::json!({
                "command": payload.command.clone(),
            }),
...
async fn get_remote_state(
    ...
    let e2e_meta = resolve_e2e_client_meta(
        query.session_id.clone(),
        query.client_id.clone(),
        query.client_label.clone(),
        query.client_kind.clone(),
        E2EClientKind::Remote,
    );
    state.app_state_sync.record_server_e2e_event(
        "server_remote_bootstrap_served",
        snapshot_payload(
            state.app_state_sync.as_ref(),
        ...
        e2e_meta.session_id.as_deref(),
        Some(e2e_meta.client_id.as_str()),
        Some(e2e_meta.client_label.as_str()),
        Some(e2e_meta.client_kind.clone()),
    );
...
async fn get_e2e_config(State(state): State<AppState>) -> Json<serde_json::Value> {
    Json(
        serde_json::to_value(state.app_state_sync.get_e2e_config()).unwrap_or_else(|_| {
            serde_json::json!({
                "enabled": false
            })
        }),
...
async fn start_e2e_session(
    State(state): State<AppState>,
    Json(request): Json<E2ESessionStartRequest>,
) -> Json<serde_json::Value> {
    let response = state
        .app_state_sync
        .start_e2e_session(request.scenario_name);
    Json(serde_json::to_value(response).unwrap_or_else(|_| serde_json::json!({})))
}

async fn end_e2e_session(
    State(state): State<AppState>,
    AxumPath(session_id): AxumPath<String>,
    Json(request): Json<E2ESessionEndRequest>,
) -> Result<Json<serde_json::Value>, (StatusCode, Json<ApiErrorResponse>)> {
    state
        .app_state_sync
        .end_e2e_session(&session_id, request.status, request.failure_reason)
        .map_err(|error| ApiErrorResponse::bad_request(&error))?;
    Ok(Json(serde_json::json!({ "status": "ok" })))
}

async fn get_e2e_session_events(
    State(state): State<AppState>,
    AxumPath(session_id): AxumPath<String>,
) -> Result<Json<serde_json::Value>, (StatusCode, Json<ApiErrorResponse>)> {
    let events = state
        .app_state_sync
        .get_e2e_session_events(&session_id)
        .ok_or_else(|| ApiErrorResponse::not_found("E2E session not found"))?;
    Ok(Json(
        serde_json::to_value(events).unwrap_or_else(|_| serde_json::json!([])),
    ))
}

async fn get_e2e_session_summary(
    State(state): State<AppState>,
    AxumPath(session_id): AxumPath<String>,
) -> Result<Json<serde_json::Value>, (StatusCode, Json<ApiErrorResponse>)> {
    let summary = state
        .app_state_sync
        .get_e2e_session_summary(&session_id)
        .ok_or_else(|| ApiErrorResponse::not_found("E2E session not found"))?;
    Ok(Json(
...
async fn handle_e2e_probe(
    State(state): State<AppState>,
    AxumPath(session_id): AxumPath<String>,
    Json(mut probe): Json<E2EProbeEvent>,
) -> Json<serde_json::Value> {
    probe.session_id = session_id;
    state.app_state_sync.record_e2e_probe(probe);
    Json(serde_json::json!({ "status": "ok" }))
}
...
async fn handle_e2e_report(
    ...
    if let Some(active_session_id) = state.app_state_sync.get_active_e2e_session_id() {
        let snapshot = serde_json::to_value(state.app_state_sync.get_e2e_state_snapshot(None))
            .unwrap_or_else(|_| serde_json::json!({}));
        state.app_state_sync.record_e2e_probe(E2EProbeEvent {
            session_id: active_session_id,
            client_id: "visualizer".to_string(),
            client_label: "visualizer".to_string(),
            client_kind: E2EClientKind::Visualizer,
            event_type: "window_state_snapshot".to_string(),
            ts: current_unix_ms(),
...
async fn state_events(
    ...
    state.app_state_sync.record_server_e2e_event(
        ...
        snapshot_payload(
            ...
            serde_json::json!({
                ...
                "sessionStartMs": query.session_start_ms,
            }),
        ),
        e2e_meta.session_id.as_deref(),
        Some(e2e_meta.client_id.as_str()),
    ...
    let disconnect_guard = Arc::new(SseDisconnectGuard {
        app_state_sync: state.app_state_sync.clone(),
        session_id: e2e_meta.session_id.clone(),
        client_id: e2e_meta.client_id.clone(),
        client_label: e2e_meta.client_label.clone(),
        client_kind: e2e_meta.client_kind.clone(),
        transport: "desktop",
    });
    ...
    let state_stream = BroadcastStream::new(rx_state)
        ...
        .map({
            ...
            move |broadcast_state: BroadcastState| -> Result<Event, Infallible> {
                let _keep_guard = &disconnect_guard;
    ...
    let command_stream = BroadcastStream::new(rx_command)
        ...
        .map({
            let disconnect_guard = disconnect_guard.clone();
            move |command: RemoteCommand| -> Result<Event, Infallible> {
                let _keep_guard = &disconnect_guard;
                Ok(Event::default()
                    .event("command")
    ...
    let connected_event =
        futures::stream::once(async move { Ok(Event::default().event("connected").data("{}")) });

    // Prepend with initial state
    let initial_event = futures::stream::once(async move {
        Ok(Event::default().event("state").data(initial_state_payload))
    });
...
async fn remote_state_events(
    ...
    let state_stream = BroadcastStream::new(rx_state)
        ...
        .map({
            let disconnect_guard = disconnect_guard.clone();
            move |remote_state| -> Result<Event, Infallible> {
                let _keep_guard = &disconnect_guard;
                Ok(Event::default()
                    .event("state")
    ...
    let sse = Sse::new(connected_event.chain(initial_event).chain(state_stream)).keep_alive(
        KeepAlive::new()
            .interval(Duration::from_secs(5))
            .text("ping"),
    );
...
mod tests {
    ...
    fn test_api_error_response_serialization() {
        ...
        let expected =
            r#"{"error":"Test error","code":"TEST_ERROR","details":null,"request_id":null}"#;
    ...
    fn test_compact_broadcast_state_strips_history_and_text_style_payloads() {
        ...
        assert_eq!(
            compact_state.message_stats["msg-1"]["lastTriggered"],
            123456
        );
        assert!(compact_state.message_stats["msg-1"]
            .get("history")
            .is_none());
    ...
    fn prop_error_response_format_consistency(scenario: ErrorScenario) -> bool {
        ...
        let status_code_matches_error_code = match response.code.as_str() {
            ...
            "INTERNAL_ERROR" | "RESOURCE_RESOLUTION_ERROR" => {
    ...
    fn prop_error_response_never_html_fallback(error_msg: String, error_code: String) -> bool {
        ...
        if let Ok(json_string) = serde_json::to_string(&error_response) {
            ...
            let contains_html = html_markers
                .iter()
                .any(|marker| json_string.contains(marker));
    ...
    fn prop_error_response_completeness(scenario: ErrorScenario) -> bool {
        ...
        let has_valid_code_format = response
            .code
            .chars()
    ...
    fn create_test_folder_with_files(files: Vec<TestImageFile>) -> Result<TempDir, std::io::Error> {
        ...
        for file in files {
            let file_path = temp_dir
                .path()
                .join(format!("{}.{}", file.name, file.extension));
    ...
    fn prop_mixed_folder_filters_correctly() {
        ...
        assert_eq!(
            found_media_files, expected_count,
