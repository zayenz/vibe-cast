...
use vibe_cast_models::{BroadcastState, DeviceType, PlaybackCommand, PlaybackControlState};
...
impl Arbitrary for ValidPlaybackCommand {
    fn arbitrary(g: &mut Gen) -> Self {
        ...
        let message_id =
            valid_message_ids[usize::arbitrary(g) % valid_message_ids.len()].to_string();
...
impl MockConnectedDevice {
    ...
    async fn try_receive_updates(
        &mut self,
        timeout_duration: Duration,
    ) -> Result<Vec<BroadcastState>, String> {
    ...
    fn received_update_after(&self, after_time: Instant) -> bool {
        self.received_states
            .iter()
            .any(|(_, receive_time)| *receive_time > after_time)
    }
}
...
async fn test_state_broadcast_consistency_async(
    ...
    for mock_device in &mut mock_devices {
        let _ = mock_device
            .try_receive_updates(Duration::from_millis(50))
            .await;
    ...
    let all_devices_received_broadcast =
        devices_received_broadcast == device_config.device_count as usize;
    let all_devices_have_correct_state =
        devices_with_correct_state == device_config.device_count as usize;
...
fn prop_state_broadcast_consistency_command_types(command_type: u8) -> TestResult {
    let rt = tokio::runtime::Runtime::new().unwrap();
    rt.block_on(async { test_command_type_broadcast_consistency_async(command_type).await })
}
...
async fn test_command_type_broadcast_consistency_async(command_type: u8) -> TestResult {
    ...
    let (main_command, setup_commands) = match command_type % 4 {
        ...
        2 => {
            ...
            let main = PlaybackCommand::Pause {
                device_id,
                timestamp,
            };
            (main, vec![setup])
        ...
        3 => {
            ...
            let main = PlaybackCommand::Resume {
                device_id,
                timestamp,
            };
    ...
    for mock_device in &mut mock_devices {
        match mock_device
            .try_receive_updates(Duration::from_millis(150))
            .await
        {
            Ok(_) => {
                if mock_device.received_update_after(command_start_time) {
...
#[quickcheck]
fn prop_state_broadcast_consistency_late_connecting_devices(
    device_connect_delay: u8,
) -> TestResult {
...
async fn test_late_connecting_device_broadcast_consistency_async(
    connect_delay_ms: u8,
) -> TestResult {
    let app_state = Arc::new(AppStateSync::new());
    ...
    match (early_final_state, late_final_state) {
        (Some(early_state), Some(late_state)) => {
            let early_consistent =
                states_equivalent(&early_state.playback_control, &expected_stop_state);
            let late_consistent =
                states_equivalent(&late_state.playback_control, &expected_stop_state);
            let states_match =
                states_equivalent(&early_state.playback_control, &late_state.playback_control);
...
mod unit_tests {
    ...
    async fn test_mock_connected_device_basic_functionality() {
        ...
        let updates = mock_device
            .try_receive_updates(Duration::from_millis(100))
            .await;
    ...
    async fn test_broadcast_consistency_single_command() {
        ...
        for device in &mut devices {
            ...
            assert!(states_equivalent(
                &latest_state.unwrap().playback_control,
                &expected_state
            ));
        }
    }
