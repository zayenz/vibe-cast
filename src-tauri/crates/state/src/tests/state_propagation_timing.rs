...
impl MockDevice {
    ...
    async fn try_receive_state_update(
        &mut self,
...
fn prop_state_propagation_timing_start_command(
    ...
    rt.block_on(async { test_state_propagation_timing_async(device_config, test_command).await })
}
...
fn prop_state_propagation_timing_scalability(device_count: u8) -> TestResult {
    ...
    let rt = tokio::runtime::Runtime::new().unwrap();
    rt.block_on(async { test_state_propagation_scalability_async(device_count).await })
}
...
fn prop_state_propagation_timing_rapid_changes(change_count: u8) -> TestResult {
    ...
    let rt = tokio::runtime::Runtime::new().unwrap();
    rt.block_on(async { test_rapid_state_changes_async(change_count).await })
}
...
mod unit_tests {
    ...
    async fn test_mock_device_basic_functionality() {
        ...
        let result = mock_device
            .try_receive_state_update(Duration::from_millis(100))
            .await;
        assert!(result.is_ok());
